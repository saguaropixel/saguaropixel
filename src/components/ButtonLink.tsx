...
export function ButtonLink({
  ...
  return (
    <PrismicNextLink
      className={clsx(
        'button-cutout group mx-4 inline-flex items-center bg-gradient-to-b from-25% to-75% bg-[length:100%_400%] font-bold lowercase transition-[filter,background-position] duration-300 hover:bg-bottom rounded-full px-8', // Added rounded-md here
        size === 'sm' && 'gap-2.5 py-2 text-base',
        size === 'md' && 'gap-3 px-1 text-lg ~py-2.5/3',
        size === 'lg' && '~text-lg/2xl ~gap-3/4 ~px-1/2 ~py-3/4',
        color === 'green' &&
          'from-brand-green to-brand-red text-brand-white hover:text-brand-black',
        color === 'yellow' &&
          'from-brand-yellow to-brand-white text-brand-black hover:text-brand-black',
        color === 'red' &&
